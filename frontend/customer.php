...
// Get current order if user is logged in
$currentOrder = null;
$orderItems = [];
if (isLoggedIn()) {
    $orderQuery = "SELECT 
    ...
    if ($currentOrder) {
        ...
        $stmt->bind_param("i", $currentOrder['ID']);
        $stmt->execute();
        ...
        <a href="index.php?page=cart" class="btn-primary">
                        ...
                        <div>
                            ...
                            <div style="display: flex; gap: 24px; margin-top: 20px; flex-wrap: wrap;">
                                <span style="color: #3b7455; display: flex; align-items: center; gap: 8px;">
                                    <i class="fas fa-circle-check" style="color: #007a3e;"></i> <?php echo $stats['open_stalls']; ?>+ food stalls
                                </span>
                                <span style="color: #3b7455; display: flex; align-items: center; gap: 8px;">
                                    <i class="fas fa-clock" style="color: #007a3e;"></i> Real-time stock
                                </span>
                                <span style="color: #3b7455; display: flex; align-items: center; gap: 8px;">
                                    <i class="fas fa-star" style="color: #007a3e;"></i> <?php echo number_format($ratingStats['avg_rating'], 1); ?> avg rating
                                </span>
                            </div>
                        </div>
            ...
            <div class="wrapper">
                ...
                <div class="section-header" id="menu">
                    ...
                    <span style="margin-left: auto; background: #e3f4ea; padding: 6px 16px; border-radius: 40px; font-size: 0.95rem; color: #007a3e; font-weight: 500; display: flex; align-items: center; gap: 6px; border: 1px solid #cae3d6;">
                    ...
                    <div class="stall-card">
                        <div class="stall-header">
                            <div style="display: flex; align-items: center; gap: 10px;">
                                ...
                                <span style="background: <?php echo $restaurant['is_open'] ? '#e3f4ea' : '#fee9e9'; ?>; padding: 4px 10px; border-radius: 40px; font-size: 0.7rem; font-weight: 600; color: <?php echo $restaurant['is_open'] ? '#007a3e' : '#b13e3e'; ?>;">
                                    <?php echo $restaurant['is_open'] ? 'Open' : 'Closed'; ?>
                                </span>
                            </div>
                            <span class="rating">
                                <i class="fas fa-star"></i> <?php echo number_format($restaurant['avg_rating'], 1); ?> 
                                <span style="color: #8faa9a; font-weight: 400; margin-left: 4px;">(<?php echo $restaurant['rating_count']; ?>)</span>
                            </span>
                        </div>
                        <div class="menu-grid">
                            <?php 
                            ...
                            <div class="menu-row" title="<?php echo htmlspecialchars($item['description'] ?? ''); ?>">
                                ...
                                <?php else: ?>
                                    <span class="avail-tag not-available"><i class="fas fa-times-circle"></i> Sold Out</span>
                                <?php endif; ?>
                            </div>
                            <?php endwhile; ?>
                        </div>
                        <div class="availability-summary">
                            <span><i class="fas fa-utensils"></i> <?php echo $restaurant['available_items_count']; ?> items available</span>
                            <span class="available-count"><?php echo $restaurant['available_items_count']; ?>/<?php echo $restaurant['total_items_count']; ?> available</span>
                        </div>
                        <a href="index.php?page=restaurant&id=<?php echo $restaurant['ID']; ?>" class="btn-secondary" style="width: 100%; margin-top: 15px; text-decoration: none;">
                ...
                <div class="track-card" id="track">
                    <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 25px; flex-wrap: wrap;">
                        <div style="background: #e3f4ea; width: 60px; height: 60px; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                            <i class="fas fa-clipboard-list" style="font-size: 1.8rem; color: #007a3e;"></i>
                        </div>
                        <div>
                            <h3 style="margin: 0; font-size: 1.5rem;">Your Order #<?php echo $currentOrder['queue_number']; ?></h3>
                            <p style="color: #3b7455; margin-top: 5px;">
                                Estimated pick-up: 
                                <?php 
                                $pickupTime = date('g:i A', strtotime($currentOrder['order_date'] . ' + 30 minutes'));
                                echo $pickupTime;
                                ?>
                            </p>
                        </div>
                        <span style="margin-left: auto; background: #daf1e2; padding: 10px 25px; border-radius: 40px; font-size: 1rem; font-weight: 600; color: #007a3e; display: flex; align-items: center; gap: 8px;">
                            <span class="status-dot" style="width: 10px; height: 10px; background: #007a3e; border-radius: 50%; display: inline-block;"></span>
                            <?php 
                            $statusText = [
                                'P' => 'Pending',
                                'PR' => 'Preparing',
                    ...
                    <div style="background: #f9fffc; border-radius: 24px; padding: 20px;">
                        <p style="font-weight: 600; margin-bottom: 16px; color: #1a4d31; display: flex; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                            <span><i class="fas fa-receipt" style="color: #007a3e;"></i> Items (<?php echo count($orderItems); ?>)</span>
                            <span style="color: #007a3e;">Total: <?php echo formatPrice($currentOrder['total_amount']); ?></span>
                        </p>
                        
                        <div style="display: flex; flex-direction: column; gap: 10px;">
                            <?php foreach($orderItems as $item): ?>
                            <div class="order-item" style="background: white; border-radius: 16px; padding: 12px 16px; border: 1px solid #e0f0e8;">
                                <span class="order-item-name">
                                    <span style="font-weight: 700;"><?php echo $item['quantity']; ?>x</span> 
                                    <?php echo htmlspecialchars($item['item_name']); ?> 
                                    <span style="color: #5f8b74;">(<?php echo htmlspecialchars($item['restaurant_name']); ?>)</span>
                                </span>
                                <span class="order-price"><?php echo formatPrice($item['price_at_time']); ?></span>
                                <span class="avail-tag order-avail status-badge confirmed" style="width: auto;">Confirmed</span>
                            </div>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <div class="order-progress">
                            <div class="progress-step">
                                <div class="step-circle <?php echo in_array($currentOrder['status'], ['P', 'PR', 'R', 'C']) ? 'active' : 'inactive'; ?>">
                                    <i class="fas fa-check"></i>
                                </div>
                                <span class="step-label">Order Placed</span>
                            </div>
                            <div class="progress-step">
                                <div class="step-circle <?php echo in_array($currentOrder['status'], ['PR', 'R', 'C']) ? 'active' : 'inactive'; ?>">
                                    <i class="fas fa-fire"></i>
                                </div>
                                <span class="step-label">Preparing</span>
                            </div>
                            <div class="progress-step">
                                <div class="step-circle <?php echo in_array($currentOrder['status'], ['R', 'C']) ? 'active' : 'inactive'; ?>">
                                    <i class="fas fa-bag-shopping"></i>
                                </div>
                                <span class="step-label">Ready</span>
                            </div>
                            <div class="progress-step">
                                <div class="step-circle <?php echo $currentOrder['status'] == 'C' ? 'active' : 'inactive'; ?>">
                ...
                <div class="track-card" style="text-align: center; padding: 40px;" id="track">
                    <i class="fas fa-clipboard-list" style="font-size: 3rem; color: #007a3e; opacity: 0.5; margin-bottom: 15px;"></i>
                    <h3 style="margin-bottom: 10px;">No Active Orders</h3>
                    <p style="color: #3b7455; margin-bottom: 20px;">Hungry? Browse our stalls and place your first order!</p>
                ...
                <div class="rating-summary" style="justify-content: space-between; flex-wrap: wrap;">
                    <div style="display: flex; align-items: center; gap: 20px;">
                        ...
                        <div>
                            <span style="font-size:2.2rem; font-weight:700;"><?php echo number_format($ratingStats['avg_rating'], 1); ?></span> / 5 
                            <span style="color: #3b7455;">· <?php echo $ratingStats['total_reviews']; ?> total reviews</span>
                        </div>
                    </div>
                    <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                        <div style="text-align: center;"><span style="font-weight:700;">5★</span> <span style="color:#3b7455;">(<?php echo $ratingStats['five_star']; ?>)</span></div>
                        <div style="text-align: center;"><span style="font-weight:700;">4★</span> <span style="color:#3b7455;">(<?php echo $ratingStats['four_star']; ?>)</span></div>
                        <div style="text-align: center;"><span style="font-weight:700;">3★</span> <span style="color:#3b7455;">(<?php echo $ratingStats['three_star']; ?>)</span></div>
                        <div style="text-align: center;"><span style="font-weight:700;">2★</span> <span style="color:#3b7455;">(<?php echo $ratingStats['two_star']; ?>)</span></div>
                    </div>
                </div>

                <!-- Quick Order CTA -->
                <div style="background: linear-gradient(135deg, #007a3e, #1e7547); border-radius: 40px; padding: 40px; margin: 40px 0; color: white; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 20px;">
                    <div>
                    ...
                    <a href="#menu" class="btn-primary" style="background: white; color: #007a3e; text-decoration: none;">
                        <i class="fas fa-bag-shopping"></i> Order Now
                    </a>
                </div>
            </div>
            <footer class="footer-note">
                <i class="fas fa-mobile-alt"></i> UniCanteen Customer · Real‑time availability · <?php echo $stats['open_stalls']; ?>+ stalls · Cashless payment
            </footer>
        </section>
    </div>
    <script>
